...
`npm run ingest` embeds these locally (Transformers.js) and also indexes captured `sites` and
`submissions`, so Ask answers over both open data and the user's own data.
