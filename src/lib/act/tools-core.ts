...
/**
 * The Locus geo tools, written once as pure functions with no app/AI-SDK dependencies, so the same
 * code runs in the in-app agent (via `tools.ts` → `aiTools`) AND in the standalone MCP server
 * (`packages/locus-mcp`). Each is `run(input) => { summary, data, features }` over a free source
 * (Nominatim, Overpass, OpenRouteService, Open-Meteo, SunCalc), with a small in-memory TTL cache.
 * `features` is GeoJSON the map renders; `summary`/`data` are what the agent/LLM observes.
 *
 * Reads `process.env.ORS_API_KEY` directly (no DB/env validation) so the MCP server stays portable.
 */
...
async function isochroneRun({ lng, lat, minutes, profile }: z.infer<typeof isoInput>): Promise<ToolRun> {
  if (!ORS_KEY) return { summary: "Isochrones need ORS_API_KEY.", data: { error: "no_key" }, features: [] };
